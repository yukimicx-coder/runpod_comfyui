...
- アプリケーション側の受信バッファ総量を 1 GiB 未満に制限する。
...
HTTP受信チャンクのサイズを `io_chunk_size` として固定する。
受信チャンクは、HTTPライブラリから返されたチャンクと書き込み直前の参照が一時的に重なる可能性がある。
そのため、同時ストリーム数は安全係数2を含めて次で計算する。

```text
max_streams = floor(buffer_limit_bytes / (2 * io_chunk_size))
```

ただし、httpx内部バッファ等の余裕を確保するため、デフォルトの `buffer_limit` は512 MiB以下とする。
`max_streams` が0になる設定はエラーにする。

この制限はアプリケーションの受信バッファを対象とする。OSのページキャッシュはプロセス内バッファではない。
